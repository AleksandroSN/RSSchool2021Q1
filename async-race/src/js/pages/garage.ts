...
export class Garage extends BaseComponent {
  ...
  listenerForCreateCar() {
    this.generateSection.createCarForm.element.addEventListener(
      "submit",
      async (evt: Event) => {
        evt.preventDefault();
  ...
  listenerForUpdateCar() {
    this.generateSection.updateCarForm.element.addEventListener(
      "submit",
      async (evt: Event) => {
        evt.preventDefault();
  ...
  listenGenerateCarsBtn() {
    this.generateSection.generateCarBtn.element.addEventListener(
      "click",
      async () => {
  ...
  listenRaceBtn() {
    this.generateSection.raceBtn.element.addEventListener("click", async () => {
      ...
      winner.then(async (x) => {
        ...
        this.raceSection.element.insertAdjacentHTML(
          "beforeend",
          announcWinner(x.carName, x.time)
        );
      });
    });
  }
