...
import { DESC, Order } from "../interfaces-and-types/types";
...
export class Pagination extends BaseComponent {
  ...
  listenPaginationBtn(button: Button, direction: string) {
    button.element.addEventListener("click", async () => {
      ...
      if (this.page === "garagePage") {
        ...
        await this.winnersPage.createWinners(this.currentPage, "wins", DESC as Order);
  ...
  checkPage() {
    if (this.page === "garagePage") {
      ...
      GetWinners(this.currentPage, 10, "wins", DESC as Order).then((response) => {
