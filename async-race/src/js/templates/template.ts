...
export const currentPage = (count: number): string => {
  return `<p class="garage__pagination">Page #<span class="garege__page">${count}</span></p>`;
};

export const createCar = (name: string, color: string): string => {
  return `<span class="garage__car-name">${name}</span>
          <div class="garage__track">
            <button class="btn__start" type="button">D</button>
            <button class="btn__stop" type="button" disabled>P</button>
            <div class="car">
              <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="170px" height="81px" viewBox="0 0 170 81"
